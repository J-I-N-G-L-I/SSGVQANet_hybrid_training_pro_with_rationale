...
class SceneToTextAttentionLayer(nn.Module):
    ...
    def forward(self, text, scene):
        ...
        # queries来自text，keys和values来自scene
        ...
        # 对新的文本再做一次自注意力，允许文本中不同的词语在具备视觉上下文后，再次进行内部的信息交流和整合
...
# 与上一个功能相反，让场景（视觉）去融合文本信息，可理解为模型再观察图像中各物体时，会参考文本信息，从而更好地理解图像内容
class TextToSceneAttentionLayer(nn.Module):
    ...
    def forward(self, text, scene):
        ...
        # queries来自scene，keys和values来自text
        ...
        # 对新的场景再做一次自注意力，允许场景中不同的物体在具备文本上下文后，再次进行内部的信息交流和整合
...
class VisualBertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        # 片段嵌入。区分不同句子或片段，例如在问答任务中，问题和答案可以被视为不同的片段
        self.token_type_embeddings = nn.Embedding(
            config.type_vocab_size, config.hidden_size
        )

        # early fusion: 在进入transformer之前，就对scene和文本进行初步的融合
        self.scene_text_layers = nn.ModuleList(
            [
                # SceneToTextAttentionLayer(scene_dim=18, text_dim=1024, dropout_rate=0.1)
                # 此处交互的仅是前18维，代表模型先让物体的语义和空间信息去和文本进行对齐和融合，得到文本感知的场景信息
        ...
        # === [PATCH A-1] Temporal (frame) position embeddings ===
        # 向后兼容：若 config 没有这些字段，则用默认值
        self.max_video_frames = getattr(config, "max_video_frames", 64)
        self.use_temporal_position_embeddings = getattr(config, "use_temporal_position_embeddings", True)
        self.visual_temporal_position_embeddings = nn.Embedding(
            self.max_video_frames, config.hidden_size
        )
        # === [PATCH A-1 END] ===
    ...
    def forward(
            ...
            visual_frame_ids=None,  # [PATCH A-2] 新增：每个视觉token对应的帧编号(0..F-1)
    ):
        ...
        embeddings += position_embeddings # 初步的文本嵌入，bert标准操作
        ...
        # 将原始视觉特征（后512）和经过文本增强的语义/空间特征（前18）拼接，得到全新的视觉嵌入
        ...
        if visual_embeds is not None:
            ...
            # === [PATCH A-3] Temporal position embeddings by frame id ===
            if self.use_temporal_position_embeddings and (visual_frame_ids is not None):
                if visual_frame_ids.dtype != torch.long:
                    visual_frame_ids = visual_frame_ids.long()
                # 防御性截断到 [0, max_video_frames-1]
                visual_frame_ids = visual_frame_ids.clamp_(min=0, max=self.max_video_frames - 1)
                visual_temporal_embeddings = self.visual_temporal_position_embeddings(visual_frame_ids)
            else:
                visual_temporal_embeddings = 0.0
            # === [PATCH A-3 END] ===

            # 合成视觉token最终embedding（多加一项 temporal）
            visual_embeddings = (
                    visual_embeds
                    + visual_position_embeddings
                    + visual_token_type_embeddings
                    + visual_temporal_embeddings
            )
...
# transformer encoder内部的标准自注意力机制，与huggingface中的BERT几乎完全相同
...
class VisualBertModel(VisualBertPreTrainedModel):
    ...
    def forward(
        ...
        visual_frame_ids: Optional[torch.LongTensor] = None,  # [PATCH B-1]
        ...
        embedding_output = self.embeddings(
            ...
            visual_frame_ids=visual_frame_ids,  # [PATCH B-2]
        )

        # bypass 将前如此的输出拆回文本和视觉两部分，文本部分经过transformer编码后再与视觉部分拼接送入一个额外的transformer layer
        # 这种设计可能是在探索late fusion的效果，与标准的早期融合作对比
...
# 在核心模型VisualBertModel的基础上添加特定的任务头，构建可以直接用于训练和评估的模型
@add_start_docstrings(
    """
...
class VisualBertForPreTraining(VisualBertPreTrainedModel):
    ...
    def forward(
        ...
        return VisualBertForPreTrainingOutput(
            loss=total_loss, # 预训练通过两个任务的损失（labels 和 sentence_image_labels）来更新所有参数
...
# VQA 任务，输入是问题文本和图像的视觉嵌入，输出是对预定义答案集合中每个答案的得分
@add_start_docstrings(
    """
...
class VisualBertForQuestionAnswering(VisualBertPreTrainedModel):
    ...
    def forward(
        ...
        # Get the index of the last text token（即 [SEP] 分隔符之前的最后一个有效token的索引）
        ...
        # gather函数根据指定的索引从输入张量中收集元素，这里是为了从序列输出中提取每个输入序列的最后一个文本token的表示
        # 这是一种池化替代策略，因为BERT的池化层通常使用[CLS]标记的表示，但在VQA任务中，最后一个文本token可能包含更相关的信息
        ...
        loss = None
        if labels is not None:
            # KL散度损失用于衡量两个概率分布之间的差异，这里用于处理多标签分类任务
