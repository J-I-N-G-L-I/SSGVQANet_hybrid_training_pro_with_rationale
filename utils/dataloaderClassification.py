...
# 用于针对性分析，根据ana_type等类别进行分类，例如location问题、counting问题
class SSGVQAClassification_full_roi_analysis(Dataset):
    ...
    def __getitem__(self, idx):
        vid = self.vqas[idx][0].split("/")[3] # 视频id
        fid = self.vqas[idx][0].split("/")[-1] # 帧id
        ...
        # 对象级特征： 14类别 + 4坐标 + 512视觉特征维度 = 530
        # 18: 对应512 维度的视觉特征部分
        # 这行代码代表：用全局图像特征visual_features_pix替换掉每个物体自己的视觉特征部分，即[自己类别，自己坐标，全局视觉特征]
        # 迫使模型更多依赖类别和空间位置信息来回答问题，而不是物体的具体外观
