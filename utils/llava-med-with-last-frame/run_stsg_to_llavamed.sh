#!/bin/bash
#SBATCH --job-name=export_stsg_llava_last_frame
...
OUTPUT_ROOT="${OUTPUT_ROOT:-/mnt/scratch/sc232jl/LLaVA-Med/stsg_temporal_eval_last_frame}"
...
echo "== START export STSG QA to LLaVA-Med format (last frame) =="
...
srun -u python utils/llava-med-with-last-frame/export_stsg_to_llavamed.py \
  ...
  --use_rationale \
  --output_questions stsg_temporal_eval_questions_last_frame.jsonl \
  --frame_ext "${FRAME_EXT}"

echo "== DONE export STSG QA to LLaVA-Med format (last frame) =="
